...
public interface ChallengeRepository extends JpaRepository<Challenge, Long> {
    ...
    @Query("select c from Challenge c Where c.member = :member and c.member.memberStatus = 1")
    List<Challenge> findAllByMember(Member member);

    @Query("select c from Challenge c " +
            "Where c.categoryName = :categoryName and c.challengeStatus = true and c.challengeProgress = 1 " +
            "ORDER BY c.modifiedAt DESC")
    Page<Challenge> findAllByCategoryNameOrderByModifiedAtDesc(CategoryName categoryName, Pageable pageable);

    @Query("select c from Challenge c " +
            "WHERE c.challengeTitle like %?1% and c.challengeStatus = true and c.challengeProgress = 1 " +
            "ORDER BY c.modifiedAt DESC")
